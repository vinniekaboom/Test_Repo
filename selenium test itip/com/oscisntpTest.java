...
public class oscisntpTest {
	...
	public void test() {
		...
		try {
			...
			System.out.println("<OSC-IS NTP Failure>");
			fail("***Error: OSC-IS National Transfer Program is unavailable***");
		}
		...
		try {
			...
			Snap.take(driver, APP);
			System.out.println("<OSC-IS NTP Failure>");
			fail("***Error: OSC-IS NTP Eligibility page failed***");
		}
		...
		try {
			...
			Snap.take(driver, APP);
			System.out.println("<OSC-IS NTP Failure>");
			fail("***Error: OSC-IS NTP logout failed***");
		}
